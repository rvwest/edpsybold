...
<section class="dan-media" aria-label="Media coverage">
    ...
    <?php if ( $articles ) : ?>
        <div class="dan-media__cards" id="dan-media-carousel">
            ...
            ?>
                <article class="dan-media__card">
                    <?php if ( $img ) : ?>
                        <img
                            src="<?php echo esc_url( $img['url'] ); ?>"
                            alt="<?php echo esc_attr( $img['alt'] ); ?>"
                            class="dan-media__card-img"
                            loading="lazy"
                        />
                    <?php endif; ?>

                    <?php if ( $article['article_outlet'] ) : ?>
                        <p class="dan-media__card-outlet"><?php echo esc_html( $article['article_outlet'] ); ?></p>
                    <?php endif; ?>

                    <?php if ( $article['article_headline'] ) : ?>
                        <h3 class="dan-media__card-headline"><?php echo esc_html( $article['article_headline'] ); ?></h3>
                    <?php endif; ?>

                    <?php if ( $url ) : ?>
                        <a
                            href="<?php echo esc_url( $url ); ?>"
                            class="dan-media__card-link"
                            target="_blank"
                            rel="noopener noreferrer"
                        >
                            Read article <i class="far fa-arrow-right" aria-hidden="true"></i>
                        </a>
                    <?php endif; ?>
                </article>
            <?php endforeach; ?>
